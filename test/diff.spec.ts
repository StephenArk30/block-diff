...
/** 序列化为先序规范形式：{id, 树内parentId, props} 数组（顺序即先序） */
...
/** round-trip：diff 后应用 ops，结果应与新树完全一致（结构 + 顺序 + props） */
...
describe('空树', () => {
  ...
  it('空 → 树：全部为 add，父先于子', () => {
    const tree: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: { n: 1 }, parentId: 'R' },
      { id: 'a1', props: { n: 2 }, parentId: 'A' },
      { id: 'B', props: { n: 3 }, parentId: 'R' },
    ];
  ...
  it('树 → 空：全部为 delete，子先于父', () => {
    const tree: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'a1', props: {}, parentId: 'A' },
    ];
    const ops = roundTrip(tree, []);
...
describe('无变化', () => {
  it('完全相同（含扁平输入顺序不同）→ 无操作', () => {
    const a: IBlock<P>[] = [
      { id: 'R', props: { n: 1 }, parentId: 'ext' },
      { id: 'A', props: { n: 2 }, parentId: 'R' },
      { id: 'B', props: { n: 3 }, parentId: 'R' },
    ];
    const b: IBlock<P>[] = [
      { id: 'B', props: { n: 3 }, parentId: 'R' },
...
describe('update', () => {
  const oldTree: IBlock<P>[] = [
    { id: 'R', props: { n: 0 }, parentId: 'ext' },
    { id: 'A', props: { n: 1 }, parentId: 'R' },
    { id: 'B', props: { n: 2 }, parentId: 'R' },
    { id: 'a1', props: { n: 3 }, parentId: 'A' },
  ];

  it('单个 props 变化', () => {
    const next = oldTree.map((b) => (b.id === 'A' ? { ...b, props: { n: 99 } } : b));
    const ops = roundTrip(oldTree, next);
    expect(ops).toEqual([{ type: 'update', id: 'A', props: { n: 99 } }]);
  });

  it('多个 props 变化（深嵌套）', () => {
    const next = oldTree.map((b) =>
      b.id === 'R' || b.id === 'a1' ? { ...b, props: { text: 'x' } } : b
    );
    const ops = roundTrip(oldTree, next);
  ...
  it('equal 判定相等时不发 update', () => {
    const next = oldTree.map((b) => (b.id === 'A' ? { ...b, props: { n: 1 } } : { ...b }));
    expect(diff(oldTree, next)).toEqual([]);
  });

  it('equal 判定不等时即使看起来相似也发 update（自定义 equal 语义）', () => {
    const next = oldTree.map((b) => (b.id === 'A' ? { ...b, props: { n: 1, extra: true } } : { ...b }));
    const ops = diff(oldTree, next);
    expect(ops).toEqual([{ type: 'update', id: 'A', props: { n: 1, extra: true } }]);
  });
});
...
describe('add', () => {
  const base = (): IBlock<P>[] => [
    { id: 'R', props: {}, parentId: 'ext' },
    { id: 'A', props: {}, parentId: 'R' },
    { id: 'B', props: {}, parentId: 'R' },
    { id: 'C', props: {}, parentId: 'R' },
  ];

  it('末尾追加', () => {
    const next = [...base(), { id: 'D', props: { n: 1 }, parentId: 'R' }];
    const ops = roundTrip(base(), next);
    expect(ops).toEqual([{ type: 'add', id: 'D', parentId: 'R', before: undefined, block: next[4] }]);
  });

  it('开头插入（锚点为第一个兄弟）', () => {
    const next = [base()[0], { id: 'Z', props: {}, parentId: 'R' }, ...base().slice(1)];
    const ops = roundTrip(base(), next);
    expect(ops).toEqual([{ type: 'add', id: 'Z', parentId: 'R', before: 'A', block: { id: 'Z', props: {}, parentId: 'R' } }]);
  });

  it('中间插入', () => {
    const next = [
      base()[0], base()[1],
      { id: 'M', props: {}, parentId: 'R' },
      base()[2], base()[3],
    ];
    const ops = roundTrip(base(), next);
    expect(ops).toEqual([{ type: 'add', id: 'M', parentId: 'R', before: 'B', block: { id: 'M', props: {}, parentId: 'R' } }]);
  });

  it('连续追加多个：顺序保持', () => {
    const next = [...base(), { id: 'D', props: {}, parentId: 'R' }, { id: 'E', props: {}, parentId: 'R' }];
    const ops = roundTrip(base(), next);
  ...
  it('在锚点前连续插入多个：顺序保持', () => {
    const next = [
      base()[0],
      { id: 'X', props: {}, parentId: 'R' },
      { id: 'Y', props: {}, parentId: 'R' },
      ...base().slice(1),
    ];
    const ops = roundTrip(base(), next);
    expect(count(ops, 'add')).toBe(2);
  ...
  it('嵌套新增：父块先于子块 add', () => {
    const next = [
      ...base(),
      { id: 'D', props: {}, parentId: 'R' },
      { id: 'd1', props: {}, parentId: 'D' },
      { id: 'd2', props: {}, parentId: 'D' },
    ];
    const ops = roundTrip(base(), next);
    const addIds = ops.filter((o) => o.type === 'add').map((o) => o.id);
  ...
  it('新增整棵子树到中间位置', () => {
    const next = [
      base()[0], base()[1],
      { id: 'S', props: {}, parentId: 'R' },
      { id: 's1', props: { n: 7 }, parentId: 'S' },
      base()[2], base()[3],
    ];
    const ops = roundTrip(base(), next);
    expect(count(ops, 'add')).toBe(2);
...
describe('delete', () => {
  ...
  it('删除带子树的中间节点：整个子树都被删除', () => {
    const oldTree: IBlock<P>[] = [
      ...
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'a1', props: {}, parentId: 'A' },
      { id: 'a2', props: {}, parentId: 'A' },
      { id: 'a1x', props: {}, parentId: 'a1' },
      { id: 'B', props: {}, parentId: 'R' },
    ];
  ...
  it('删父留子：子节点 reparent，move 先于 delete', () => {
    const oldTree: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'keep', props: {}, parentId: 'A' },
      { id: 'B', props: {}, parentId: 'R' },
    ];
    const next: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'B', props: {}, parentId: 'R' },
      { id: 'keep', props: {}, parentId: 'R' },
    ];
    const ops = roundTrip(oldTree, next);
...
describe('move：同父重排', () => {
  ...
  it('深层子节点重排', () => {
    const oldTree: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'x1', props: {}, parentId: 'A' },
      { id: 'x2', props: {}, parentId: 'A' },
      { id: 'x3', props: {}, parentId: 'A' },
    ];
...
describe('move：跨父', () => {
  ...
  it('跨父移动 + 同父重排组合', () => {
    ...
    const next: IBlock<P>[] = [
      ...
      base()[4], base()[5],
      { id: 'a1', props: {}, parentId: 'B' },
      { id: 'b2', props: {}, parentId: 'B' },
    ];
    const ops = roundTrip(base(), next);
    expect(count(ops, 'move')).toBe(1);
...
describe('子树 diff（根带外部 parentId）', () => {
  it('根层级结构变化不影响 diff 正确性', () => {
    const oldTree: IBlock<P>[] = [
      { id: 'root', props: { n: 1 }, parentId: 'page-1' },
      { id: 'A', props: { n: 2 }, parentId: 'root' },
      { id: 'B', props: { n: 3 }, parentId: 'root' },
    ];
    const next: IBlock<P>[] = [
      { id: 'root', props: { n: 1 }, parentId: 'page-2' }, // 外部父变了，不参与 diff
      { id: 'B', props: { n: 3 }, parentId: 'root' },
      { id: 'A', props: { n: 2 }, parentId: 'root' },
    ];
    const ops = roundTrip(oldTree, next);
  ...
  it('旧根与新根 id 不同：删旧根 + 加新根', () => {
    const oldTree: IBlock<P>[] = [
      { id: 'oldRoot', props: {}, parentId: 'page' },
      { id: 'A', props: {}, parentId: 'oldRoot' },
    ];
    const next: IBlock<P>[] = [
      { id: 'newRoot', props: {}, parentId: 'page' },
      { id: 'A', props: {}, parentId: 'newRoot' },
    ];
    const ops = roundTrip(oldTree, next);
  ...
  it('森林：多个顶层根之间移动', () => {
    const oldTree: IBlock<P>[] = [
      { id: 't1', props: {}, parentId: 'ext' },
      { id: 't2', props: {}, parentId: 'ext' },
      { id: 't3', props: {}, parentId: 'ext' },
      { id: 'x', props: {}, parentId: 't1' },
    ];
    const next: IBlock<P>[] = [
      { id: 't3', props: {}, parentId: 'ext' },
      { id: 't1', props: {}, parentId: 'ext' },
      { id: 't2', props: {}, parentId: 'ext' },
      { id: 'x', props: {}, parentId: 't2' },
    ];
    const ops = roundTrip(oldTree, next);
...
describe('组合场景', () => {
  it('update + add + delete + move 混合', () => {
    const oldTree: IBlock<P>[] = [
      ...
      { id: 'A', props: { n: 1 }, parentId: 'R' },
      { id: 'B', props: { n: 2 }, parentId: 'R' },
      { id: 'C', props: { n: 3 }, parentId: 'R' },
      { id: 'c1', props: { n: 4 }, parentId: 'C' },
    ];
    const next: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'NEW', props: { n: 9 }, parentId: 'R' },
      { id: 'C', props: { n: 3 }, parentId: 'R' },
      { id: 'B', props: { n: 20 }, parentId: 'R' },
      { id: 'c1', props: { n: 40 }, parentId: 'R' }, // 从 C 下移到 R 下
    ];
    const ops = roundTrip(oldTree, next);
  ...
  it('换父节点的子树内部还有结构变化', () => {
    const oldTree: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'X', props: {}, parentId: 'A' },
      { id: 'x1', props: {}, parentId: 'X' },
      { id: 'x2', props: {}, parentId: 'X' },
      { id: 'B', props: {}, parentId: 'R' },
    ];
    const next: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'B', props: {}, parentId: 'R' },
      { id: 'X', props: {}, parentId: 'B' }, // X 换父
      { id: 'x2', props: {}, parentId: 'X' }, // X 内部重排
      { id: 'x1', props: {}, parentId: 'X' },
    ];
    const ops = roundTrip(oldTree, next);
...
describe('op 输出顺序', () => {
  it('顺序为 update → move/add（交错）→ delete', () => {
    ...
    const next: IBlock<P>[] = [
      { id: 'R', props: { n: 100 }, parentId: 'ext' },
      { id: 'B', props: { n: 200 }, parentId: 'R' },
      { id: 'A', props: { n: 1 }, parentId: 'R' },
      { id: 'N', props: {}, parentId: 'R' },
    ];
    const ops = roundTrip(oldTree, next);
  ...
  it('move 的目标父为新增块时，该父的 add 先于 move', () => {
    ...
    const next: IBlock<P>[] = [
      { id: 'R', props: {}, parentId: 'ext' },
      { id: 'A', props: {}, parentId: 'R' },
      { id: 'NEW', props: {}, parentId: 'R' },
      { id: 'x', props: {}, parentId: 'NEW' }, // x 换父到新增块 NEW 下
    ];
    const ops = roundTrip(oldTree, next);
...
describe('异常输入', () => {
  ...
  it('parentId 指向不存在的块：视为顶层', () => {
    const tree: IBlock<P>[] = [
      { id: 'A', props: {}, parentId: 'ghost' },
      { id: 'B', props: {}, parentId: 'A' },
    ];
